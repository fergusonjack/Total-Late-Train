...
public class Main {
		...
		for (TrainServicesField train : arrivals.getTrainServicesField()) {
			...
			if (train.getEtaField().equals("On time") || train.getEtaField().equals("Delayed")) {
				...
				try {
					...
					if (greatestDelay.containsKey(train.getrsidField())
							&& ((delay = getTimeDiff(staHour, staMinute, etaHour, etaMinute)) > greatestDelay
									.get(train.getrsidField()))) {
					...
					if (!checked.contains(train.getrsidField())
							&& (getTimeDiff(currentHour, currentMinute, etaHour, etaMinute) < 6)) {
						if (train.getAdhocAlertsField() != null){ addToReasons(train.getAdhocAlertsField());}
	...
	public static void resetReasons() {
		try {
			FileWriter writer = new FileWriter("reasons.txt");
			writer.write("");
			writer.close();
		} catch (IOException e) {
	...
	public static void addToReasons(String toAdd) {
		...
		try {
			...
			Scanner s = new Scanner(new File("reasons.txt"));
			while (s.hasNext()) {
				list.add(s.next());
			}
			s.close();
			...
			System.out.println("error with adding extra to the reasons file");
		}
	}
